...
@Component
@Order(2)
class CorrIdResponseTraceFilter : GlobalFilter {

    private val log = LoggerFactory.getLogger(CorrIdResponseTraceFilter::class.java)

    @Autowired
    private lateinit var filterUtility: FilterUtility

    override fun filter(exchange: ServerWebExchange, chain: GatewayFilterChain): Mono<Void> {
        val correlationId = exchange.request.headers[HeaderNames.CORR_ID]
            ?.get(0)
            ?: filterUtility.generateCorrelationId()

        log.debug("Updated corr-id for the response: $correlationId")
        exchange.response.headers.add(HeaderNames.CORR_ID, correlationId)
